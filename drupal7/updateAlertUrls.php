...
$queryFindUrls = '
  SELECT %2$s, %3$s FROM %1$s WHERE %2$s RLIKE \'[[:<:]]https://alerts\.berkeleywellness\.com[[:>:]]\'
';
...
if (mysqli_connect_errno()) {
  echo "Failed to connect to MySQL: {${mysqli_connect_error()}}";
    ...
    foreach ($migrations as $table => $columns) {
        ...
        $foundUrl = $connection->query($select);
        if ($foundUrl->num_rows) {
            echo "== Found {$foundUrl->num_rows} matches rows in {$table}.{$columns["set"]} {$type} \n";
            echo "==================================================================\n";
            $rowCount = 1;
            while ($stringWithUrl = $foundUrl->fetch_assoc()) {
                $content = $stringWithUrl[$columns['set']];
                if (array_key_exists("{$table}.{$columns["set"]}", $serialized)) {
                    ...
                    if (is_string($unserializedContent)) {
                        $migratedContent = replaceHttpsAlertUrlsToHttp($unserializedContent);
                            ...
                            if(is_array($value1)) {
                                foreach ($value1 as $key2 => &$value2) {
                                    if (preg_match('/^php/', $key2) &&
                                        ...
                                        $value2["php_output"] = replaceHttpsAlertUrlsToHttp($value2["php_output"]);
                                    }
                                }
                            }
                        }
                    ...
                    $migratedContent = replaceHttpsAlertUrlsToHttp($content);
                }
                echo "{$rowCount} of {$foundUrl->num_rows} ";
                if ($migratedContent !== $content) {
                    $whereConditionals = [];
                    foreach ($columns['where'] as $column) {
                        $escapedValue = mysqli_real_escape_string($connection, $stringWithUrl[$column]);
